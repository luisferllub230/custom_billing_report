...
export class BillingTopCustomersChart extends Component {
    static template = "custom_billing_report.BillingTopCustomersChart";
    static props = {
    ...
    _renderChart() {
        ...
        const top = (this.props.topCustomers || []).slice(0, 5);
        const labels = top.map((c) => c.partner_name);
        const paid = top.map((c) => c.amount_paid || 0);
        const pending = top.map((c) => c.amount_pending || 0);
